...
import type { GameState, Player, Clue, Vote, GameSettings } from '../types/game';
...
function calculateImposterCount(playerCount: number): number {
  ...
  return Math.floor(playerCount / 4); // 1 imposter per 4 players for larger groups
}
...
export function useGame() {
  ...
  const createGame = useCallback((playerNames: string[], secretWord?: string) => {
    ...
    // Calculate number of imposters based on player count
    const numImposters = calculateImposterCount(playerNames.length);
  ...
  const checkWinConditions = useCallback((eliminatedPlayer: Player) => {
    ...
    if (eliminatedPlayer.role === 'imposter') {
      ...
      const remainingImposters = gameState.players.filter(p => 
        p.role === 'imposter' && !p.isEliminated
      );
      
      if (remainingImposters.length === 0) {
        // All imposters eliminated - crew wins
        setGameState(prev => prev ? {
          ...prev,
          winner: 'crew',
        } : null);
      ...
      // Imposters win if crew count equals or is less than imposter count
